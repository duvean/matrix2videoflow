...
class GraphNodeItem(QGraphicsRectItem):
    ...
    def _add_pixel_sort_controls(self, y_pos):
        ...
        for w in (threshold, strength, direction):
            w.setStyleSheet("background:#111827;color:white;border:1px solid #475569;padding:1px;")
...
QTreeWidget::item:selected {
    ...
    def open_graph_fullscreen(self):
        ...
        expanded.scale(1.5, 1.5)
        expanded.setMinimumSize(1400, 900)
        expanded.fit_graph()
    ...
    def build_prerender(self):
        ...
        self.prerender_source_frame_ids = [ids[min(len(ids) - 1, i // stride)] for i in
                                           range(len(self.prerender_frames))]
    ...
    def update_playback_visuals(self):
        ...
        self.playback_progress.setValue(
            int((self.current_playback_index / total) * 1000) if self.prerender_frames else 0)
    ...
    def start_mask_edit_mode(self):
        ...
        if mask is None:
            ...
            m['mask'] = cv2.resize(mask, (self.current_preview_frame.shape[1], self.current_preview_frame.shape[0]),
